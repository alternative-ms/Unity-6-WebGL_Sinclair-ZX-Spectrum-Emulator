...
namespace Speculator.Core
{
    public class Alu
    {
        ...
        internal static bool IsBytePositive(byte b) => (b & 0x80) == 0;

        private static bool IsWordPositive(int a) => (a & 0x8000) == 0;

        internal static sbyte FromTwosCompliment(byte b)
        {
            if ((b & 0x80) != 0) return (sbyte)(-128 + (b & 0x7f));
        ...
        internal static bool IsEvenParity(byte b) => (CountBits(b) & 1) == 0;
        ...
        {
            if (isAddition) return (!signA && !signB && signResult) || (signA && signB && !signResult);
        ...
        private static bool IsHalfCarry8(byte a, byte b, bool carry, bool isAddition)
        {
            ...
            if (isAddition) return (((a & 0x0f) + (b & 0x0f) + c) & 0x10) != 0;
        ...
        {
            var c = (byte)(carry ? 1 : 0);
            if (isAddition) return (((a & 0x0fff) + (b & 0x0fff) + c) & 0x1000) != 0;

            return (((a & 0x0fff) - (b & 0x0fff) - c) & 0x1000) != 0;
        }

        private static bool IsCarry8(int unclippedResult) => (unclippedResult & 0x100) != 0;

        private static bool IsCarry16(int unclippedResult) => (unclippedResult & 0x10000) != 0;
    }
}