...
namespace Speculator.Core
{
    public partial class CPU
    {
        ...
        private int Tick()
        {
            ...
            if (instruction != null) return ExecuteInstruction(instruction);
            ...
            try
            {
                switch (opcodeByte)
                {
                    ...
                    case 0xFD:
                        ...
                        if (!m_opcodeWarningIssued) UnityEngine.Debug.LogWarning($"Ignoring {opcodeByte:X2} prefix for opcode {MainMemory.ReadAsHexString(TheRegisters.PC, 4, true)} (Disabling future warnings).");
                    ...
                    case 0xED:
                        ...
                        else
                        {
                            ...
                            if (!m_opcodeWarningIssued) UnityEngine.Debug.LogWarning($"Ignoring ED prefix for opcode {MainMemory.ReadAsHexString(TheRegisters.PC, 4, true)} (Disabling future warnings).");
                        }
        ...
        private int ExecuteInstruction(Instruction instruction, bool incrementR = true)
        {
            ...
            switch (instruction.Id)
            {
                ...
                // replaced ThePortHandler.Out(regs.Main.C, hlMem);  -->>  DoOut(regs.Main.C, hlMem);
                case Z80Instructions.InstructionID.OUTI:
                    {
                        regs.Main.B = TheAlu.DecAndSetFlags(regs.Main.B);
                        var hlMem = MainMemory.Peek(regs.Main.HL);
                        DoOut(regs.Main.C, hlMem);
                        regs.Main.HL++;
