...
namespace Speculator.Core
{
    public class Instruction
    {
        ...
        public bool StartsWithOpcodeBytes(Memory mainMemory, ushort addr)
        {
            ...
            var span = mainMemory.MemoryData.AsSpan(addr);
            if (span.Length < m_totalOpcodeLength) return false;

            // Compare contiguous fixed prefix using vectorized SequenceEqual.
            if (m_fixedPrefix.Length != 0 && !span[..m_fixedPrefix.Length].SequenceEqual(m_fixedPrefix)) return false;
            ...
            {
                var p = other[i];
                if (span[p.Offset] != p.Value) return false;
            }
        ...
        private void EnsureOpcodePattern()
        {
            ...
            for (byte i = 0; i < tokens.Length; i++)
            {
                ...
                if (!isVar)
                {
                    ...
                    if (!seenVariable) prefix.Add(val);
                    else others.Add((i, val));
                }

                if (isVar) seenVariable = true;
            }
