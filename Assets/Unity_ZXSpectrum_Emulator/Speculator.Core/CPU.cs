...
namespace Speculator.Core
{
    public partial class CPU : ViewModelBase
    {
        ...
        public bool IsDebuggerActive
        {
            ...
            set
            {
                if (SetField(ref m_isDebuggerActive, value)) ClockSync.Reset();
            }
        }
        ...
        private void RunLoop()
        {
            ...
            long tStatesAtLastSync = TStatesSinceCpuStart;

            while (!m_shutdownRequested)
            {
                if (IsDebuggerActive)
                {
                    if (!m_debuggerTickEvent.WaitOne(100)) continue;
                }
                else
                {
                    // We synchronize only once every 20 m s (approximately one Spectrum frame)
                    // this will reduce the load on the system.
                    if (TStatesSinceCpuStart - tStatesAtLastSync > 60000)
                    {
                        ClockSync.SyncWithRealTime();
                        tStatesAtLastSync = TStatesSinceCpuStart;
                    }
                }
        ...
        private void DoOut(ushort port, byte value)
        {
            // Send to hardware port (for keyboard/border)
            ThePortHandler?.Out((byte)(port & 0xFF), value);

            // if port is 0xFE (254) — this is sound
            if ((port & 0x00FF) == 0xFE)
            {
                // bit 4 (0x10) — this is speaker
                byte speakerBit = (byte)((value & 0x10) >> 4);
                m_soundHandler?.SetSpeakerState(speakerBit);
            }
        }
        ...
        private void doCPI()
        {
            ...
            if (TheRegisters.HalfCarryFlag) v--;
